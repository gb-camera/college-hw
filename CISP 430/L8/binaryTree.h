...
//nodeCount and leavesCount implemented
...
//Following are the implemented functions
template <class elemType>
int binaryTreeType<elemType>::nodeCount(binaryTreeNode<elemType>* p) const {
    //First check if the node is empty
    if (p == NULL) { return 0; }
    //If the node isn't empty, count it as a node
    //Recurse through each node
    else { return 1 + nodeCount(p->llink) + nodeCount(p->rlink); }
    //When the count function reaches a leaf, it returns 1 + 0 + 0;
    //Once the leaves exit execution, a node with two children at the second to last level of the tree will return 3
    //And so on and so forth; by the end of the execution, we should have counted each node individually
}


template <class elemType>
int binaryTreeType<elemType>::leavesCount(binaryTreeNode<elemType> *p) const{
    //Again, check if the node is empty
    if (p == NULL) { return 0; }
    //If the node isn't empty, check if it itself is a leaf
    if (p->llink == NULL && p->rlink == NULL) { return 1; }
    //If not, recurse through the tree until we find the leaves
    else { return leavesCount(p->llink) + leavesCount(p->rlink); }
    //When each count function returns from its execution, it returns 1 for each leaf in the tree,
    //so by the end of the execution, we should have all 1s for each leaf added up
}
